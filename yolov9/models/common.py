...
class DetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights='yolo.pt', backup=None, device=torch.device('cpu'), dnn=False, data=None, fp16=False, fuse=True):
        ...
        if pt:  # PyTorch
            model = attempt_load(weights if isinstance(weights, list) else w, backup=backup, device=device, inplace=True, fuse=fuse)
    ...
    def _model_type(p='path/to/model.pt'):
        ...
        return types if len(types)==13 else types + [triton]
